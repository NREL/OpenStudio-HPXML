...
def run_workflow(basedir, rundir, hpxml, debug, timeseries_output_freq, timeseries_outputs, skip_validation, add_comp_loads,
                 output_format, building_id, ep_input_format, detailed_schedules_type, local_clock_time_type)
  ...
  unless local_clock_time_type.nil?
    args['timeseries_local_time'] = local_clock_time_type
  end
  update_args_hash(measures, measure_subdir, args)
...
OptionParser.new do |opts|
  ...
  opts.on('--add-local-clock-time TYPE', ['DST', 'UTC'], 'Add local clock time column (DST, UTC)') do |t|
    options[:local_clock_time_type] = t
  end
...
success = run_workflow(basedir, rundir, options[:hpxml], options[:debug], timeseries_output_freq, timeseries_outputs,
                       ...
                       options[:ep_input_format], options[:detailed_schedules_type], options[:local_clock_time_type])
