...
class WorkflowSimulations1Test < Minitest::Test
  def test_simulations1
    ...
    # Run simulations BEFORE base-hvac-dse.xml; the remaining simulations are run using test_simulations2.rb
    # This distributes the simulations across two CI jobs for faster turnaround time.
    split_at_file = Dir["#{sample_files_dir}/*.xml"].sort.find_index { |f| f.include? 'base-hvac-dse.xml' }
