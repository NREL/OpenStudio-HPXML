...
def run_simulation_tests(xmls)
  ...
  Parallel.map(xmls, in_threads: Parallel.processor_count) do |xml|
    ...
    all_annual_results[xml_name], all_bill_results[xml_name], monthly_results = results
  ...
  return all_annual_results, all_bill_results
end
...
def _run_xml(xml, worker_num, apply_unit_multiplier = false, annual_results_1x = nil, monthly_results_1x = nil)
  ...
  annual_results = _get_simulation_annual_results(annual_csv_path)
  ...
  return annual_results, bill_results, monthly_results
end

def _get_simulation_annual_results(annual_csv_path)
