...
def _verify_outputs(rundir, hpxml_path, results, hpxml, unit_multiplier)
  ...
  File.readlines(File.join(rundir, 'run.log')).each do |message|
    ...
    if (not hpxml_bldg.hvac_controls.empty?) && ((hpxml_bldg.hvac_controls[0].seasons_heating_begin_month != 1) || (hpxml_bldg.hvac_controls[0].seasons_cooling_begin_month != 1))
