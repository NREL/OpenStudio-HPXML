...
class HPXMLTest < MiniTest::Test
  ...
  def test_invalid
    ...
    expected_error_msgs = { 'boiler-invalid-afue.xml' => ['Expected AnnualHeatingEfficiency[Units="AFUE"]/Value to be less than or equal to 1'],
                            ...
                            'invalid-duct-area-fractions.xml' => ['Expected sum(Ducts/FractionDuctArea) for DuctType="supply" to be 1 [context: /HPXML/Building/BuildingDetails/Systems/HVAC/HVACDistribution/DistributionSystemType/AirDistribution, id: "HVACDistribution"]',
                                                                  'Expected sum(Ducts/FractionDuctArea) for DuctType="return" to be 1 [context: /HPXML/Building/BuildingDetails/Systems/HVAC/HVACDistribution/DistributionSystemType/AirDistribution, id: "HVACDistribution"]'],
                            ...
                            'invalid-skylights-physical-properties.xml' => ["Could not lookup UFactor and SHGC for skylight 'SkylightSouth'."],
                            ...
                            'invalid-windows-physical-properties.xml' => ["Could not lookup UFactor and SHGC for window 'WindowSouth'."],
  ...
  def _verify_outputs(rundir, hpxml_path, results, hpxml)
    ...
    File.readlines(File.join(rundir, 'eplusout.err')).each do |err_line|
      ...
      next if err_line.include?('setupIHGOutputs: Output variables=Space Other Equipment') && err_line.include?('are not available') # FIXME: Should be able to remove this with updated E+ rc release
      next if err_line.include? 'Actual air mass flow rate is smaller than 25% of water-to-air heat pump coil rated air flow rate.' # FIXME: Investigate this
