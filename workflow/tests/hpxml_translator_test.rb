...
class HPXMLTest < MiniTest::Test
  ...
  def _verify_outputs(rundir, hpxml_path, results, hpxml)
    ...
    File.readlines(File.join(rundir, 'eplusout.err')).each do |err_line|
      ...
      next if err_line.include? 'Coil control failed to converge for AirLoopHVAC:UnitarySystem' # FIXME: Double-check
      next if err_line.include? 'Coil control failed for AirLoopHVAC:UnitarySystem' # FIXME: Double-check
      next if err_line.include? 'sensible part-load ratio out of range error continues' # FIXME: Double-check
      next if err_line.include? 'Iteration limit exceeded in calculating sensible part-load ratio error continues' # FIXME: Double-check
