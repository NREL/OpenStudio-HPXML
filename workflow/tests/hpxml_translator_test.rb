...
class HPXMLTest < MiniTest::Test
  ...
  def _verify_outputs(rundir, hpxml_path, results, hpxml)
    ...
    File.readlines(File.join(rundir, 'eplusout.err')).each do |err_line|
      ...
      # HP defrost curves
      if hpxml.heat_pumps.select { |hp| [HPXML::HVACTypeHeatPumpAirToAir, HPXML::HVACTypeHeatPumpMiniSplit, HPXML::HVACTypeHeatPumpPTHP, HPXML::HVACTypeHeatPumpRACwithReverseCycle].include? hp.heat_pump_type }.size > 0
