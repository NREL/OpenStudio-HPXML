...
- Adds a `UseHERSForHeatPumps` sizing option that is equivalent to the previous `UseMaxLoadForHeatPumps` option (heat pump nominal capacity .
...
- Adds a new `UseHERSForHeatPumps` sizing option to retain the previous `UseMaxLoadForHeatPumps` behavior (nominal heating capacity equals heating design load).
