...
class HPXMLtoOpenStudioHotWaterApplianceTest < MiniTest::Test
  ...
  def test_base
    ...
    water_sens = -262.507
    water_lat = 266.358
    assert_in_epsilon(water_sens, get_oe_kwh(model, Constants.ObjectNameWater + ' Sensible'), 0.001)
    ...
    assert_in_epsilon(water_lat, get_oe_kwh(model, Constants.ObjectNameWater + ' Latent'), 0.001)
  ...
  def test_dhw_multiple_distribution
    args_hash = {}
    args_hash['hpxml_path'] = File.absolute_path(File.join(sample_files_dir, 'base-dhw-multiple-distribution.xml'))
    model, hpxml = _test_measure(args_hash)
