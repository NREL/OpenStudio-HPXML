...
class HPXMLtoOpenStudioLightingTest < MiniTest::Test
  ...
  def test_lighting_led
    ['base-misc-defaults.xml'].each do |hpxml_name|
      args_hash = {}
      ...
      # Check interior lighting
      int_kwh_yr = get_kwh_per_year(model, Constants.ObjectNameInteriorLighting)
      assert_in_delta(1746, int_kwh_yr, 1.0)

      # Check garage lighting
      grg_kwh_yr = get_kwh_per_year(model, Constants.ObjectNameGarageLighting)
      assert_equal(0, grg_kwh_yr.to_f)

      # Check exterior lighting
      ext_kwh_yr = get_kwh_per_year(model, Constants.ObjectNameExteriorLighting)
      assert_in_delta(137, ext_kwh_yr, 1.0)
    end
  end

  def test_lighting_number_of_units
    ['base-lighting-number-of-units.xml'].each do |hpxml_name|
      args_hash = {}
      args_hash['hpxml_path'] = File.absolute_path(File.join(sample_files_dir, hpxml_name))
      model, hpxml = _test_measure(args_hash)
      ...
      # Check interior lighting
      int_kwh_yr = get_kwh_per_year(model, Constants.ObjectNameInteriorLighting)
      assert_in_delta(1958, int_kwh_yr, 1.0)
      ...
      # Check exterior lighting
      ext_kwh_yr = get_kwh_per_year(model, Constants.ObjectNameExteriorLighting)
      assert_in_delta(235, ext_kwh_yr, 1.0)
    end
  end

  def test_lighting_none
    ['base-lighting-none.xml'].each do |hpxml_name|
      args_hash = {}
      args_hash['hpxml_path'] = File.absolute_path(File.join(sample_files_dir, hpxml_name))
      model, hpxml = _test_measure(args_hash)
      ...
      # Check interior lighting
      int_kwh_yr = get_kwh_per_year(model, Constants.ObjectNameInteriorLighting)
      assert_equal(0, int_kwh_yr.to_f)
      ...
      # Check exterior lighting
      ext_kwh_yr = get_kwh_per_year(model, Constants.ObjectNameExteriorLighting)
      assert_equal(0, ext_kwh_yr.to_f)
    end
  end

  def test_lighting_exterior_holiday
    ['base.xml',
