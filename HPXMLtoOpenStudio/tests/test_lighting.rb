...
class HPXMLtoOpenStudioLightingTest < MiniTest::Test
  ...
  def test_lighting_kwh_per_year
    ...
    # Check interior lighting
    int_kwh_yr = hpxml.lighting_groups.find { |lg| lg.location == HPXML::LocationInterior }.map { |lg| lg.kwh_per_year }
    assert_in_delta(int_kwh_yr, get_kwh_per_year(model, Constants.ObjectNameInteriorLighting), 1.0)

    # Check exterior lighting
    ext_kwh_yr = hpxml.lighting_groups.find { |lg| lg.location == HPXML::LocationExterior }.map { |lg| lg.kwh_per_year }
