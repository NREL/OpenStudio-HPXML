...
class HPXMLtoOpenStudioLightingTest < Minitest::Test
  ...
  def test_lighting
    ...
    assert_in_delta(98, get_kwh_per_year(model, Constants.ObjectNameLightingExterior).round, 1.0)
  end
  ...
  def test_lighting_kwh_per_year
    ...
    assert_in_delta(ext_kwh_yr, get_kwh_per_year(model, Constants.ObjectNameLightingExterior).round, 1.0)
  end
