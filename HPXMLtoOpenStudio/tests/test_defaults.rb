...
class HPXMLtoOpenStudioDefaultsTest < MiniTest::Test
  ...
  def _test_default_room_air_conditioner_ptac_values(cooling_system, shr, cooling_capacity, cooling_efficiency_eer, integrated_heating_system_efficiency_percent = nil)
    assert_equal(shr, cooling_system.cooling_shr)
    if cooling_capacity.nil?
      assert(cooling_system.cooling_capacity > 0)
    else
    ...
    if integrated_heating_system_efficiency_percent.nil?
      assert_nil(cooling_system.integrated_heating_system_efficiency_percent)
    else
      assert_equal(cooling_system.integrated_heating_system_efficiency_percent, integrated_heating_system_efficiency_percent)
    end
  end
