...
class HPXMLtoOpenStudioValidationTest < Minitest::Test
  ...
  def test_ruby_error_messages
    ...
    all_expected_errors = { 'battery-bad-values-max-greater-than-one' => ["Schedule value for column 'battery' must be less than or equal to 1."],
                            ...
                            'storm-windows-unexpected-window-ufactor' => ['Storm windows are currently restricted to windows with U-factor >= 0.45, while base window U-Factor was 0.33.'],
  ...
  def test_ruby_warning_messages
    ...
    all_expected_warnings.each_with_index do |(warning_case, expected_warnings), i|
      ...
      when 'schedule-file-and-weekday-weekend-multipliers'
        ...
        hpxml_bldg.header.schedules_filepaths << File.join(File.dirname(__FILE__), '../resources/schedule_files/stochastic-3-occupants.csv')
        hpxml_bldg.header.schedules_filepaths << File.join(File.dirname(__FILE__), '../resources/schedule_files/non-stochastic-occupants.csv')
