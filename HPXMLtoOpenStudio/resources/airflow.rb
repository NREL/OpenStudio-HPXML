...
module Airflow
  ...
  def self.initialize_cfis(model, vent_fans, airloop_map, unavailable_periods)
    cfis_data = { airloop: {}, t_sum_open_damper_var: {}, f_open_damper_var: {} }
    ...
    vent_fans[:mech].each do |vent_mech|
      ...
      cfis_data[:f_open_damper_var][vent_mech.id] = Model.add_ems_global_var(
        model,
      ...
      cfis_program.addLine("Set #{cfis_data[:f_open_damper_var][vent_mech.id].name} = 0")
    ...
    duct_locations.each_with_index do |duct_location, i|
      ...
      if cfis_data[:airloop].values.include? object
        ...
        if add_cfis_duct_losses
          ...
          f_open_damper_var = cfis_data[:f_open_damper_var][cfis_id]

          duct_program.addLine("If #{f_open_damper_var.name} > 0")
          ...
          duct_program.addLine("  Set #{fan_data[:rtf_var][object].name} = #{f_open_damper_var.name}") # Need to use global vars to sync duct_program and infiltration program of different calling points
  ...
  def self.apply_cfis(runner, infil_program, vent_mech_fans, cfis_data, cfis_fan_actuator, cfis_suppl_fan_actuator, fan_data)
    ...
    vent_mech_fans.each do |vent_mech|
      ...
      f_open_damper_var = cfis_data[:f_open_damper_var][vent_mech.id]
      ...
      infil_program.addLine('Set f_damper_open = 0') # Fraction of the timestep the CFIS damper is open
      infil_program.addLine("Set #{f_open_damper_var.name} = 0")
      ...
      infil_program.addLine("  Set t_fan_on = 60 - (t_min_hr_open - #{t_sum_open_damper_var.name})") # Minute of the hour at which additional ventilation mode runtime needed to meet the ventilation requirements
      infil_program.addLine('  If ((Minute+0.00001) >= t_fan_on) && (has_additional_runtime == 1)') # Check whether supply fan has to run to achieve target minutes per hour of operation
      infil_program.addLine('    Set addtl_fan_runtime = @Max (@ABS(Minute - t_fan_on)) (fan_rtf_hvac * ZoneTimeStep * 60)') # Additional ventilation mode runtime in minutes
      infil_program.addLine("    Set addtl_fan_runtime = @Min addtl_fan_runtime (t_min_hr_open - #{t_sum_open_damper_var.name})") # Make sure it's not exceeding ventilation requirements
      infil_program.addLine('    Set f_damper_open = addtl_fan_runtime / (60.0 * ZoneTimeStep)') # calculates the portion of the current timestep the damper needs to be open
      infil_program.addLine("    Set #{t_sum_open_damper_var.name} = #{t_sum_open_damper_var.name} + addtl_fan_runtime")
      infil_program.addLine("    Set #{f_open_damper_var.name} = @Max (f_damper_open - fan_rtf_hvac) 0.0") # Fraction of the timestep with additional ventilation mode runtime
      if vent_mech.cfis_addtl_runtime_operating_mode == HPXML::CFISModeAirHandler
        ...
        infil_program.addLine("    Set #{cfis_fan_actuator.name} = #{cfis_fan_actuator.name} + fan_w * #{f_open_damper_var.name}")
        ...
        if vent_mech.cfis_supplemental_fan.oa_unit_flow_rate > 0
          infil_program.addLine("    Set suppl_f = #{f_open_damper_var.name} / (suppl_Q_oa / Q_duct_oa)") # Calculate desired runtime for supplemental fan to provide remaining ventilation requirement
      ...
      infil_program.addLine('    Set addtl_fan_runtime = fan_rtf_hvac * ZoneTimeStep * 60') # Additional ventilation mode runtime in minutes
      ...
      if vent_mech.cfis_addtl_runtime_operating_mode == HPXML::CFISModeSupplementalFan
        infil_program.addLine("  Set f_damper_open = @Max (f_damper_open - #{f_open_damper_var.name}) 0.0")
      end
      infil_program.addLine('  Set QWHV_cfis_sup = QWHV_cfis_sup + (f_damper_open * Q_duct_oa)')
      infil_program.addLine('EndIf')
    end
  end
