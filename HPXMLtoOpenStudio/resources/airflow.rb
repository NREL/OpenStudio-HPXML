  ...
  def self.apply_cfis(runner, infil_program, vent_mech_fans, cfis_data, cfis_fan_actuator, cfis_suppl_fan_actuator, fan_data)
    ...
    vent_mech_fans.each do |vent_mech|
      ...
      if vent_mech.cfis_control_type == HPXML::CFISControlTypeTimer
        ...
        # Calculate target outdoor air ventilation for the hour
        infil_program.addLine('Set oa_cfm_target = f_operation * oa_cfm_ah')

        # Calculate outdoor air ventilation still needed for the hour
        infil_program.addLine("Set oa_cfm_needed = oa_cfm_target - #{sum_oa_cfm_var.name}")
        infil_program.addLine('If oa_cfm_needed > 0.00001')

        # Calculate available outdoor air ventilation during HVAC runtime
        infil_program.addLine('  Set oa_cfm_during_hvac_avail = fan_rtf_hvac * oa_cfm_ah')

        # Calculate actual outdoor air ventilation brought in during HVAC runtime
        if vent_mech.cfis_has_outdoor_air_control
          infil_program.addLine('  Set oa_cfm_during_hvac = @Min oa_cfm_during_hvac_avail oa_cfm_needed')
        else
          # Outdoor air is introduced for the entire time the HVAC system is running
          infil_program.addLine('  Set oa_cfm_during_hvac = oa_cfm_during_hvac_avail')
        end
        infil_program.addLine('  Set QWHV_cfis_sup = QWHV_cfis_sup + oa_cfm_during_hvac')
        infil_program.addLine("  Set #{sum_oa_cfm_var.name} = #{sum_oa_cfm_var.name} + oa_cfm_during_hvac")

        # Calculate additional outdoor air ventilation still needed for the hour after HVAC runtime
        infil_program.addLine('  Set oa_cfm_addtl_needed = oa_cfm_needed - oa_cfm_during_hvac')

        # Calculate outdoor air ventilation that can be brought in during subsequent timesteps of the hour if needed
        if vent_mech.cfis_addtl_runtime_operating_mode == HPXML::CFISModeAirHandler
          infil_program.addLine('  Set oa_cfm_addtl_needed = oa_cfm_addtl_needed - (((60 - Minute) / 60) * oa_cfm_ah)')
        elsif vent_mech.cfis_addtl_runtime_operating_mode == HPXML::CFISModeSupplementalFan
          infil_program.addLine('  Set oa_cfm_addtl_needed = oa_cfm_addtl_needed - (((60 - Minute) / 60) * oa_cfm_suppl)')
        elsif vent_mech.cfis_addtl_runtime_operating_mode == HPXML::CFISModeNone
          infil_program.addLine('  Set oa_cfm_addtl_needed = 0')
        end
        infil_program.addLine('  If oa_cfm_addtl_needed > 0.00001')

        if vent_mech.cfis_addtl_runtime_operating_mode == HPXML::CFISModeAirHandler
          # Air handler meets additional runtime requirement

          # Calculate available outdoor air ventilation during non-HVAC runtime
          infil_program.addLine('    Set oa_cfm_during_non_hvac_avail = (1.0 - fan_rtf_hvac) * oa_cfm_ah')

          # Calculate actual outdoor air ventilation brought in during non-HVAC runtime
          infil_program.addLine('    Set oa_cfm_during_non_hvac = @Min oa_cfm_during_non_hvac_avail oa_cfm_addtl_needed')
          infil_program.addLine('    Set QWHV_cfis_sup = QWHV_cfis_sup + oa_cfm_during_non_hvac')
          infil_program.addLine("    Set #{sum_oa_cfm_var.name} = #{sum_oa_cfm_var.name} + oa_cfm_during_non_hvac")

          # Calculate fraction of the timestep with ventilation only mode runtime
          infil_program.addLine("    Set #{f_vent_only_mode_var.name} = (oa_cfm_during_non_hvac / (@Max oa_cfm_during_non_hvac_avail 0.00001)) * (1.0 - fan_rtf_hvac)") # @Max used to prevent divide be zero
          ...
          # Calculate available outdoor air ventilation during non-HVAC runtime
          infil_program.addLine('    Set oa_cfm_during_non_hvac_avail = (1.0 - fan_rtf_hvac) * oa_cfm_suppl')

          # Calculate actual outdoor air ventilation brought in during non-HVAC runtime
          infil_program.addLine('    Set oa_cfm_during_non_hvac = @Min oa_cfm_during_non_hvac_avail oa_cfm_addtl_needed')
          if vent_mech.cfis_supplemental_fan.fan_type == HPXML::MechVentTypeSupply
            infil_program.addLine('    Set QWHV_cfis_suppl_sup = QWHV_cfis_suppl_sup + oa_cfm_during_non_hvac')
          elsif vent_mech.cfis_supplemental_fan.fan_type == HPXML::MechVentTypeExhaust
            infil_program.addLine('    Set QWHV_cfis_suppl_exh = QWHV_cfis_suppl_exh + oa_cfm_during_non_hvac')
          end
          infil_program.addLine("    Set #{sum_oa_cfm_var.name} = #{sum_oa_cfm_var.name} + oa_cfm_during_non_hvac")
          ...
          # If specified, additionally run supplemental fan when damper is open and HVAC system is running
          if vent_mech.cfis_supplemental_fan_runs_with_air_handler_fan
            infil_program.addLine('    Set f_open_damper_ah = (oa_cfm_during_hvac / (@Max oa_cfm_during_hvac_avail 0.00001)) * fan_rtf_hvac') # @Max used to prevent divide be zero
