...
class Airflow
  def self.apply(model, runner, weather, spaces, hpxml, cfa, nbeds,
    ...
    vented_dryers = hpxml.clothes_dryers.select { |cd| cd.is_vented && cd.vented_flow_rate.to_f > 0 && (HPXML::conditioned_locations - [HPXML::LocationOtherHousingUnit]).include?(cd.location) }
