  ...
  def self.apply_cfis(runner, infil_program, vent_mech_fans, cfis_data, cfis_fan_actuator, cfis_suppl_fan_actuator, fan_data)
    ...
    vent_mech_fans.each do |vent_mech|
      ...
      if vent_mech.cfis_control_type == HPXML::CFISControlTypeTimer
        ...
        infil_program.addLine('If oa_cfm_needed > 0')

        # Calculate hourly-average available outdoor air ventilation during HVAC runtime
        infil_program.addLine('  Set oa_cfm_during_hvac_avail = fan_rtf_hvac * oa_cfm_ah * ZoneTimestep')
        ...
        infil_program.addLine('  Set QWHV_cfis_sup = QWHV_cfis_sup + (oa_cfm_during_hvac / ZoneTimestep)') # Converted to timestep cfm
        ...
        if vent_mech.cfis_addtl_runtime_operating_mode == HPXML::CFISModeSupplementalFan && vent_mech.cfis_supplemental_fan_runs_with_air_handler_fan
          infil_program.addLine('  If oa_cfm_during_hvac > 0')
          infil_program.addLine('    Set f_open_damper_ah = (oa_cfm_during_hvac / oa_cfm_during_hvac_avail) * fan_rtf_hvac')
          ...
          if vent_mech.cfis_supplemental_fan.fan_type == HPXML::MechVentTypeSupply
            infil_program.addLine('    Set QWHV_cfis_suppl_sup = QWHV_cfis_suppl_sup + (f_open_damper_ah * oa_cfm_suppl / ZoneTimestep)') # Converted to timestep cfm
          elsif vent_mech.cfis_supplemental_fan.fan_type == HPXML::MechVentTypeExhaust
            infil_program.addLine('    Set QWHV_cfis_suppl_exh = QWHV_cfis_suppl_exh + (f_open_damper_ah * oa_cfm_suppl / ZoneTimestep)') # Converted to timestep cfm
          end
          infil_program.addLine('EndIf')
        end
        ...
        infil_program.addLine('  If oa_cfm_addtl_needed > 0')
        ...
        if vent_mech.cfis_addtl_runtime_operating_mode == HPXML::CFISModeAirHandler
          ...
          infil_program.addLine('    Set oa_cfm_during_non_hvac_avail = (1.0 - fan_rtf_hvac) * oa_cfm_ah * ZoneTimestep')
          ...
          infil_program.addLine('    Set QWHV_cfis_sup = QWHV_cfis_sup + (oa_cfm_during_non_hvac / ZoneTimestep)') # Converted to timestep cfm
          ...
          infil_program.addLine('    If oa_cfm_during_non_hvac > 0')
          infil_program.addLine("      Set #{f_vent_only_mode_var.name} = (oa_cfm_during_non_hvac / oa_cfm_during_non_hvac_avail) * (1.0 - fan_rtf_hvac)")
          infil_program.addLine("      Set #{cfis_fan_actuator.name} = #{cfis_fan_actuator.name} + (ah_fan_w * #{f_vent_only_mode_var.name})")
          infil_program.addLine('    EndIf')
          ...
          # Calculate hourly-average available outdoor air ventilation during non-HVAC runtime
          infil_program.addLine('    Set oa_cfm_during_non_hvac_avail = (1.0 - fan_rtf_hvac) * oa_cfm_suppl * ZoneTimestep')

          # Calculate hourly-average actual outdoor air ventilation brought in during non-HVAC runtime
          infil_program.addLine('    Set oa_cfm_during_non_hvac = @Min oa_cfm_during_non_hvac_avail oa_cfm_addtl_needed')
          if vent_mech.cfis_supplemental_fan.fan_type == HPXML::MechVentTypeSupply
            infil_program.addLine('    Set QWHV_cfis_suppl_sup = QWHV_cfis_suppl_sup + (oa_cfm_during_non_hvac / ZoneTimestep)') # Converted to timestep cfm
          elsif vent_mech.cfis_supplemental_fan.fan_type == HPXML::MechVentTypeExhaust
            infil_program.addLine('    Set QWHV_cfis_suppl_exh = QWHV_cfis_suppl_exh + (oa_cfm_during_non_hvac / ZoneTimestep)') # Converted to timestep cfm
          end
          infil_program.addLine("    Set #{sum_oa_cfm_var.name} = #{sum_oa_cfm_var.name} + oa_cfm_during_non_hvac")
