...
class Geometry
  ...
  def self.apply_occupants(model, runner, hpxml, num_occ, cfa, space, schedules_file)
    ...
    if people_sch.nil?
      ...
      runner.registerWarning("Both '#{Constants.Occupants}' schedule file and weekday fractions provided; the latter will be ignored.") if !hpxml.building_occupancy.weekday_fractions.nil?
      runner.registerWarning("Both '#{Constants.Occupants}' schedule file and weekend fractions provided; the latter will be ignored.") if !hpxml.building_occupancy.weekend_fractions.nil?
      runner.registerWarning("Both '#{Constants.Occupants}' schedule file and monthly multipliers provided; the latter will be ignored.") if !hpxml.building_occupancy.monthly_multipliers.nil?
    end
