...
class HVAC
  ...
  def self.apply_ceiling_fans(model, runner, weather, ceiling_fan, living_space, schedules_file)
    ...
    if not schedules_file.nil?
      ...
      ceiling_fan_design_level = schedules_file.calc_design_level_from_annual_kwh(col_name: Constants.CeilingFan, annual_kwh: annual_kwh)
      ceiling_fan_sch = schedules_file.create_schedule_file(col_name: Constants.CeilingFan)
    end
    if ceiling_fan_sch.nil?
      ...
      runner.registerWarning("Both '#{Constants.CeilingFan}' schedule file and weekday fractions provided; the latter will be ignored.") if !ceiling_fan.weekday_fractions.nil?
      runner.registerWarning("Both '#{Constants.CeilingFan}' schedule file and weekend fractions provided; the latter will be ignored.") if !ceiling_fan.weekend_fractions.nil?
      runner.registerWarning("Both '#{Constants.CeilingFan}' schedule file and monthly multipliers provided; the latter will be ignored.") if !ceiling_fan.monthly_multipliers.nil?
    end
