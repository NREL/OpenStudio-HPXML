  ...
  def self.apply(runner, model, epw_file, spaces, lighting_groups, lighting, eri_version, schedules_file, cfa, gfa)
    ...
    if int_kwh > 0
      ...
      if not schedules_file.nil?
        design_level = schedules_file.calc_design_level_from_annual_kwh(col_name: Constants.LightingInterior, annual_kwh: int_kwh)
        interior_sch = schedules_file.create_schedule_file(col_name: Constants.LightingInterior)
      end
      if interior_sch.nil?
        ...
        runner.registerWarning("Both '#{Constants.LightingInterior}' schedule file and weekday fractions provided; the latter will be ignored.") if !lighting.interior_weekday_fractions.nil?
        runner.registerWarning("Both '#{Constants.LightingInterior}' schedule file and weekend fractions provided; the latter will be ignored.") if !lighting.interior_weekend_fractions.nil?
        runner.registerWarning("Both '#{Constants.LightingInterior}' schedule file and monthly multipliers provided; the latter will be ignored.") if !lighting.interior_monthly_multipliers.nil?
      end
    ...
    if grg_kwh > 0
      ...
      # Create schedule
      if not schedules_file.nil?
        design_level = schedules_file.calc_design_level_from_annual_kwh(col_name: Constants.LightingGarage, annual_kwh: grg_kwh)
        garage_sch = schedules_file.create_schedule_file(col_name: Constants.LightingGarage)
      end
      if garage_sch.nil?
        ...
        runner.registerWarning("Both '#{Constants.LightingGarage}' schedule file and weekday fractions provided; the latter will be ignored.") if !lighting.garage_weekday_fractions.nil?
        runner.registerWarning("Both '#{Constants.LightingGarage}' schedule file and weekend fractions provided; the latter will be ignored.") if !lighting.garage_weekend_fractions.nil?
        runner.registerWarning("Both '#{Constants.LightingGarage}' schedule file and monthly multipliers provided; the latter will be ignored.") if !lighting.garage_monthly_multipliers.nil?
      end
    ...
    if ext_kwh > 0
      ...
      # Create schedule
      if not schedules_file.nil?
        design_level = schedules_file.calc_design_level_from_annual_kwh(col_name: Constants.LightingExterior, annual_kwh: ext_kwh)
        exterior_sch = schedules_file.create_schedule_file(col_name: Constants.LightingExterior)
      end
      if exterior_sch.nil?
        ...
        runner.registerWarning("Both '#{Constants.LightingExterior}' schedule file and weekday fractions provided; the latter will be ignored.") if !lighting.exterior_weekday_fractions.nil?
        runner.registerWarning("Both '#{Constants.LightingExterior}' schedule file and weekend fractions provided; the latter will be ignored.") if !lighting.exterior_weekend_fractions.nil?
        runner.registerWarning("Both '#{Constants.LightingExterior}' schedule file and monthly multipliers provided; the latter will be ignored.") if !lighting.exterior_monthly_multipliers.nil?
      end
    ...
    if not lighting.holiday_kwh_per_day.nil?
      ...
      # Create schedule
      if not schedules_file.nil?
        design_level = schedules_file.calc_design_level_from_daily_kwh(col_name: Constants.LightingExteriorHoliday, daily_kwh: lighting.holiday_kwh_per_day)
        exterior_holiday_sch = schedules_file.create_schedule_file(col_name: Constants.LightingExteriorHoliday)
      end
      if exterior_holiday_sch.nil?
        ...
        runner.registerWarning("Both '#{Constants.LightingExteriorHoliday}' schedule file and weekday fractions provided; the latter will be ignored.") if !lighting.holiday_weekday_fractions.nil?
        runner.registerWarning("Both '#{Constants.LightingExteriorHoliday}' schedule file and weekend fractions provided; the latter will be ignored.") if !lighting.holiday_weekend_fractions.nil?
        runner.registerWarning("Both '#{Constants.LightingExteriorHoliday}' schedule file and monthly multipliers provided; the latter will be ignored.") if !lighting.exterior_monthly_multipliers.nil?
      end
