...
class Outputs
  ...
  def self.append_sizing_results(hpxml_bldgs, results_out)
    ...
    results_out << [line_break]
    results_out << ['HVAC Design Load: Heating: Total (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.hdl_total * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Heating: Ducts (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.hdl_ducts * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Heating: Windows (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.hdl_windows * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Heating: Skylights (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.hdl_skylights * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Heating: Doors (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.hdl_doors * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Heating: Walls (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.hdl_walls * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Heating: Roofs (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.hdl_roofs * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Heating: Floors (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.hdl_floors * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Heating: Slabs (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.hdl_slabs * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Heating: Ceilings (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.hdl_ceilings * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Heating: Infiltration (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.hdl_infil * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Heating: Ventilation (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.hdl_vent * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Total (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_total * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Ducts (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_ducts * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Windows (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_windows * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Skylights (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_skylights * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Doors (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_doors * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Walls (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_walls * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Roofs (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_roofs * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Floors (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_floors * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Slabs (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_slabs * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Ceilings (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_ceilings * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Infiltration (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_infil * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Ventilation (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_vent * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: Internal Gains (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_intgains * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Sensible: AED Excursion (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_sens_aedexcursion * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Latent: Total (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_lat_total * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Latent: Ducts (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_lat_ducts * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Latent: Infiltration (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_lat_infil * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Latent: Ventilation (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_lat_vent * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
    results_out << ['HVAC Design Load: Cooling Latent: Internal Gains (Btu/h)', hpxml_bldgs.map { |hpxml_bldg| hpxml_bldg.conditioned_zones.map { |zone| zone.cdl_lat_intgains * hpxml_bldg.building_construction.number_of_units }.sum(0.0) }.sum(0.0).round(1)]
