...
class UtilityBills
  def self.get_rates_from_eia_data(runner, state_code, fuel_type, fixed_charge, marginal_rate = nil)
    ...
    if fuel_type == HPXML::FuelTypeElectricity
      ...
      if not marginal_rate.nil?
        ...
        year_ix = nil
        rows = CSV.read(File.join(File.dirname(__FILE__), '../../ReportUtilityBills/resources/data/simple_rates/Average_retail_price_of_electricity.csv'))
        rows.each do |row|
      ...
      if not marginal_rate.nil?
        ...
        # Calculate marginal & average rates from user-specified fixed charge and EIA data
        rows = CSV.read(File.join(File.dirname(__FILE__), '../../ReportUtilityBills/resources/data/simple_rates/NG_PRI_SUM_A_EPG0_PRS_DMCF_A.csv'))
        rows = rows[2..-1]
  ...
  def self.get_household_consumption(state_code, fuel_type)
    rows = CSV.read(File.join(File.dirname(__FILE__), '../../ReportUtilityBills/resources/data/simple_rates/HouseholdConsumption.csv'))
    rows.each do |row|
  ...
  def self.get_gallon_marginal_rates(filename)
    ...
    rows = CSV.read(File.join(File.dirname(__FILE__), "../../ReportUtilityBills/resources/data/simple_rates/#{filename}"))
    rows = rows[2..-1]
    headers = rows[0]
