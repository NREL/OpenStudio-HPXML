  ...
  def self.apply(model, runner, hpxml, weather, spaces, hot_water_distribution,
    ...
    if not clothes_washer.nil?
      ...
      if not schedules_file.nil?
        cw_design_level_w = schedules_file.calc_design_level_from_daily_kwh(col_name: Constants.ClothesWasher, daily_kwh: cw_annual_kwh / 365.0)
        power_cw_schedule = schedules_file.create_schedule_file(col_name: Constants.ClothesWasher)
      end
      if power_cw_schedule.nil?
        ...
        runner.registerWarning("Both '#{Constants.ClothesWasher}' schedule file and weekday fractions provided; the latter will be ignored.") if !clothes_washer.weekday_fractions.nil?
        runner.registerWarning("Both '#{Constants.ClothesWasher}' schedule file and weekend fractions provided; the latter will be ignored.") if !clothes_washer.weekend_fractions.nil?
        runner.registerWarning("Both '#{Constants.ClothesWasher}' schedule file and monthly multipliers provided; the latter will be ignored.") if !clothes_washer.monthly_multipliers.nil?
      end
    ...
    if not clothes_dryer.nil?
      ...
      cd_schedule = nil
      if not schedules_file.nil?
        cd_design_level_e = schedules_file.calc_design_level_from_annual_kwh(col_name: Constants.ClothesDryer, annual_kwh: cd_annual_kwh)
        cd_design_level_f = schedules_file.calc_design_level_from_annual_therm(col_name: Constants.ClothesDryer, annual_therm: cd_annual_therm)
        cd_schedule = schedules_file.create_schedule_file(col_name: Constants.ClothesDryer)
      end
      if cd_schedule.nil?
        ...
        runner.registerWarning("Both '#{Constants.ClothesDryer}' schedule file and weekday fractions provided; the latter will be ignored.") if !clothes_dryer.weekday_fractions.nil?
        runner.registerWarning("Both '#{Constants.ClothesDryer}' schedule file and weekend fractions provided; the latter will be ignored.") if !clothes_dryer.weekend_fractions.nil?
        runner.registerWarning("Both '#{Constants.ClothesDryer}' schedule file and monthly multipliers provided; the latter will be ignored.") if !clothes_dryer.monthly_multipliers.nil?
      end
    ...
    if not dishwasher.nil?
      ...
      if power_cw_schedule.nil?
        ...
        runner.registerWarning("Both '#{Constants.Dishwasher}' schedule file and weekday fractions provided; the latter will be ignored.") if !dishwasher.weekday_fractions.nil?
        runner.registerWarning("Both '#{Constants.Dishwasher}' schedule file and weekend fractions provided; the latter will be ignored.") if !dishwasher.weekend_fractions.nil?
        runner.registerWarning("Both '#{Constants.Dishwasher}' schedule file and monthly multipliers provided; the latter will be ignored.") if !dishwasher.monthly_multipliers.nil?
      end
    ...
    hpxml.refrigerators.each do |refrigerator|
      ...
      fridge_schedule = nil
      if not schedules_file.nil?
        fridge_col_name = refrigerator.primary_indicator ? Constants.Refrigerator : Constants.ExtraRefrigerator
    ...
    hpxml.freezers.each do |freezer|
      ...
      if not schedules_file.nil?
        freezer_design_level = schedules_file.calc_design_level_from_annual_kwh(col_name: Constants.Freezer, annual_kwh: fz_annual_kwh)
        freezer_schedule = schedules_file.create_schedule_file(col_name: Constants.Freezer)
      end
      if freezer_schedule.nil?
        ...
        runner.registerWarning("Both '#{Constants.Freezer}' schedule file and weekday fractions provided; the latter will be ignored.") if !freezer.weekday_fractions.nil?
        runner.registerWarning("Both '#{Constants.Freezer}' schedule file and weekend fractions provided; the latter will be ignored.") if !freezer.weekend_fractions.nil?
        runner.registerWarning("Both '#{Constants.Freezer}' schedule file and monthly multipliers provided; the latter will be ignored.") if !freezer.monthly_multipliers.nil?
      end
    ...
    if not cooking_range.nil?
      ...
      if not schedules_file.nil?
        ...
        cook_schedule = schedules_file.create_schedule_file(col_name: Constants.CookingRange)
      end
      if cook_schedule.nil?
        ...
        runner.registerWarning("Both '#{Constants.CookingRange}' schedule file and weekday fractions provided; the latter will be ignored.") if !cooking_range.weekday_fractions.nil?
        runner.registerWarning("Both '#{Constants.CookingRange}' schedule file and weekend fractions provided; the latter will be ignored.") if !cooking_range.weekend_fractions.nil?
        runner.registerWarning("Both '#{Constants.CookingRange}' schedule file and monthly multipliers provided; the latter will be ignored.") if !cooking_range.monthly_multipliers.nil?
      end
    ...
    if not hot_water_distribution.nil?
      ...
      if not schedules_file.nil?
        fixtures_schedule = schedules_file.create_schedule_file(col_name: Constants.HotWaterFixtures)
      end
      if fixtures_schedule.nil?
        ...
        runner.registerWarning("Both '#{Constants.HotWaterFixtures}' schedule file and weekday fractions provided; the latter will be ignored.") if !hpxml.water_heating.water_fixtures_weekday_fractions.nil?
        runner.registerWarning("Both '#{Constants.HotWaterFixtures}' schedule file and weekend fractions provided; the latter will be ignored.") if !hpxml.water_heating.water_fixtures_weekend_fractions.nil?
        runner.registerWarning("Both '#{Constants.HotWaterFixtures}' schedule file and monthly multipliers provided; the latter will be ignored.") if !hpxml.water_heating.water_fixtures_monthly_multipliers.nil?
      end
    end
    ...
    hpxml.water_heating_systems.each do |water_heating_system|
      ...
      if gpd_frac > 0
        ...
        if not schedules_file.nil?
          fx_peak_flow = schedules_file.calc_peak_flow_from_daily_gpm(col_name: Constants.HotWaterFixtures, daily_water: fx_gpd)
          dist_water_peak_flow = schedules_file.calc_peak_flow_from_daily_gpm(col_name: Constants.HotWaterFixtures, daily_water: w_gpd)
        else
        ...
        if dist_pump_annual_kwh > 0
          if not schedules_file.nil?
            dist_pump_design_level = schedules_file.calc_design_level_from_daily_kwh(col_name: Constants.HotWaterFixtures, daily_kwh: dist_pump_annual_kwh / 365.0)
          else
      ...
      if not clothes_washer.nil?
        ...
        if not gpd_frac.nil?
          ...
          if not schedules_file.nil?
            cw_peak_flow = schedules_file.calc_peak_flow_from_daily_gpm(col_name: Constants.HotWaterClothesWasher, daily_water: cw_gpd)
            water_cw_schedule = schedules_file.create_schedule_file(col_name: Constants.HotWaterClothesWasher)
          end
      ...
      if not schedules_file.nil?
        dw_peak_flow = schedules_file.calc_peak_flow_from_daily_gpm(col_name: Constants.HotWaterDishwasher, daily_water: dw_gpd)
        water_dw_schedule = schedules_file.create_schedule_file(col_name: Constants.HotWaterDishwasher)
      end
    ...
    if not hot_water_distribution.nil?
      ...
      if not schedules_file.nil?
        water_design_level_sens = schedules_file.calc_design_level_from_daily_kwh(col_name: Constants.HotWaterFixtures, daily_kwh: UnitConversions.convert(water_sens_btu, 'Btu', 'kWh') / 365.0)
        water_design_level_lat = schedules_file.calc_design_level_from_daily_kwh(col_name: Constants.HotWaterFixtures, daily_kwh: UnitConversions.convert(water_lat_btu, 'Btu', 'kWh') / 365.0)
      else
