...
class Waterheater
  def self.apply_tank(model, runner, loc_space, loc_schedule, water_heating_system, ec_adj, solar_thermal_system, eri_version, schedules_file, power_outage_periods)
  ...
  def self.apply_tankless(model, runner, loc_space, loc_schedule, water_heating_system, ec_adj, solar_thermal_system, eri_version, schedules_file, power_outage_periods)
  ...
  def self.apply_heatpump(model, runner, loc_space, loc_schedule, weather, water_heating_system, ec_adj, solar_thermal_system, living_zone, eri_version, schedules_file, power_outage_periods)
    ...
    if setpoint_schedule.nil?
      setpoint_schedule = ScheduleConstant.new(model, Constants.ObjectNameWaterHeaterSetpoint, t_set_c, Constants.ScheduleTypeLimitsTemperature, off_periods: power_outage_periods)
  ...
  def self.apply_combi(model, runner, loc_space, loc_schedule, water_heating_system, ec_adj, solar_thermal_system, eri_version, schedules_file, power_outage_periods)
    solar_fraction = get_water_heater_solar_fraction(water_heating_system, solar_thermal_system)
  ...
  def self.create_new_heater(name:, water_heating_system: nil, act_vol:, t_set_c: nil, loc_space:, loc_schedule: nil, model:, runner:, u: nil, ua:, eta_c: nil, is_dsh_storage: false, is_combi: false, schedules_file: nil, power_outage_periods: [])
    ...
    if tank_model_type == HPXML::WaterHeaterTankModelTypeStratified
      ...
      configure_stratified_tank_setpoint_schedules(new_heater, schedules_file, t_set_c, model, runner, power_outage_periods)
      ...
      configure_mixed_tank_setpoint_schedule(new_heater, schedules_file, t_set_c, model, runner, power_outage_periods)
  ...
  def self.configure_mixed_tank_setpoint_schedule(new_heater, schedules_file, t_set_c, model, runner, power_outage_periods)
    new_schedule = nil
    if not schedules_file.nil?
    ...
    if new_schedule.nil? # constant
      new_schedule = ScheduleConstant.new(model, Constants.ObjectNameWaterHeaterSetpoint, t_set_c, Constants.ScheduleTypeLimitsTemperature, off_periods: power_outage_periods)
  ...
  def self.configure_stratified_tank_setpoint_schedules(new_heater, schedules_file, t_set_c, model, runner, power_outage_periods)
    new_schedule = nil
    if not schedules_file.nil?
      new_schedule = schedules_file.create_schedule_file(col_name: SchedulesFile::ColumnWaterHeaterSetpoint)
    end
