...
class Waterheater
  ...
  def self.unmet_wh_loads_program(model, water_heating_systems, plantloop_map, showers_peak_flow)
    ...
    water_heating_systems.each do |water_heating_system|
      ...
      unmet_wh_loads_program.addLine("Set ShowerE=#{shower_flow_sensor.name} * #{showers_peak_flow[water_heating_system.id]} * 990.0 * 4183.0 * (#{mixed_setpoint_sensor.name} - #{wh_temp_sensor.name})")
      unmet_wh_loads_program.addLine('Else')
