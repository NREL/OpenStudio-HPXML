...
class Waterheater
  ...
  def self.unmet_wh_loads_program(model, water_heating_systems, plantloop_map, showers_peak_flow)
    ...
    water_heating_systems.each do |water_heating_system|
      ...
      unmet_wh_loads_program.addLine("If (#{shower_flow_sensor.name} > 0) && (#{wh_temp_sensor.name} < #{mixed_setpoint_sensor.name})")      unmet_wh_loads_program.addLine('Set ShowerSagTime=SystemTimeStep')
