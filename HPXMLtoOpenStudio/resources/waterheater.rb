...
class Waterheater
  def self.apply_tank(model, space, loc_schedule, fuel_type, cap, vol,
    ...
    new_heater = create_new_heater(name: Constants.ObjectNameWaterHeater, cap: cap, fuel: fuel_type, act_vol: act_vol, t_set: t_set, space: space, loc_schedule: loc_schedule, wh_type: HPXML::WaterHeaterTypeStorage, model: model, ua: ua, eta_c: eta_c, oncycle_p: 0.0, ef: ef)
    ...
    if not desuperheater_clg_coil.nil?
      dhw_map[sys_id] << add_desuperheater(model, t_set, new_heater, desuperheater_clg_coil, space, loc_schedule, loop)
    end
  end
  ...
  def self.apply_combi(model, runner, space, loc_schedule, vol, t_set, ec_adj,
    ...
    # Create water heater
    new_heater = create_new_heater(name: obj_name_combi, act_vol: act_vol, t_set: t_set, space: space, loc_schedule: loc_schedule, wh_type: tank_type, model: model, ua: ua)
    set_parasitic_power_for_storage_wh(water_heater: new_heater)
  ...
  def self.add_desuperheater(model, t_set, tank, desuperheater_clg_coil, space, loc_schedule, loop)
    ...
    storage_tank = create_new_heater(name: storage_tank_name, act_vol: storage_vol_actual, t_set: tank_setpoint, space: space, loc_schedule: loc_schedule, wh_type: HPXML::WaterHeaterTypeStorage, model: model, ua: assumed_ua)
  ...
  def self.create_new_heater(name:, cap: 0.0, fuel: nil, act_vol:, t_set:, space:, loc_schedule: nil, wh_type:, model:, ua:, eta_c: nil, oncycle_p: 0.0, ef: nil)
