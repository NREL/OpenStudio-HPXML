...
class Waterheater
  def self.apply_tank(model, loc_space, loc_schedule, water_heating_system, ec_adj, solar_thermal_system)
    ...
    loop.additionalProperties.setFeature('HPXML_ID', water_heating_system.id)
    ...
    new_heater.additionalProperties.setFeature('HPXML_ID', water_heating_system.id)

    loop.addSupplyBranchForComponent(new_heater)

    add_ec_adj(model, new_heater, ec_adj, loc_space, water_heating_system).each do |obj|
      obj.additionalProperties.setFeature('HPXML_ID', water_heating_system.id) unless obj.nil?
    end

    if water_heating_system.uses_desuperheater
      desuperheater = add_desuperheater(model, water_heating_system, new_heater, loc_space, loc_schedule, loop)
      desuperheater.additionalProperties.setFeature('HPXML_ID', water_heating_system.id)
    end
  end
  ...
  def self.apply_heatpump(model, runner, loc_space, loc_schedule, weather, water_heating_system, ec_adj, solar_thermal_system, living_zone)
    ...
    coil.additionalProperties.setFeature('HPXML_ID', water_heating_system.id)
    ...
    tank.additionalProperties.setFeature('HPXML_ID', water_heating_system.id)

    if water_heating_system.uses_desuperheater
      desuperheater = add_desuperheater(model, water_heating_system, tank, loc_space, loc_schedule, loop)
      desuperheater.additionalProperties.setFeature('HPXML_ID', water_heating_system.id)
    end

    # Fan:SystemModel
    fan = setup_hpwh_fan(model, obj_name_hpwh, airflow_rate)
    fan.additionalProperties.setFeature('HPXML_ID', water_heating_system.id)
    ...
    hpwh.additionalProperties.setFeature('HPXML_ID', water_heating_system.id)
  ...
  def self.apply_combi(model, runner, loc_space, loc_schedule, water_heating_system, ec_adj, solar_thermal_system)
    ...
    combi_hx.additionalProperties.setFeature('HPXML_ID', water_heating_system.id)
    ...
    new_pump.additionalProperties.setFeature('HPXML_ID', water_heating_system.id)
    ...
    new_source_manager.additionalProperties.setFeature('HPXML_ID', water_heating_system.id)
  ...
  def self.apply_combi_system_EMS(model, water_heating_systems)
    ...
    combi_sys_ids.each do |combi_sys_id|
      ...
      model.getWaterUseConnectionss.each do |wuc|
        sys_id = wuc.additionalProperties.getFeatureAsString('HPXML_ID')
        next unless (sys_id.is_initialized && sys_id.get == combi_sys_id)
      ...
      model.getWaterHeaterMixeds.each do |object|
      ...
      model.getPumpVariableSpeeds.each do |object|
        sys_id = object.additionalProperties.getFeatureAsString('HPXML_ID')
        next unless (sys_id.is_initialized && sys_id.get == combi_sys_id)
      ...
      model.getSetpointManagerScheduleds.each do |object|
        sys_id = object.additionalProperties.getFeatureAsString('HPXML_ID')
        next unless (sys_id.is_initialized && sys_id.get == combi_sys_id)
  ...
  def self.apply_solar_thermal(model, loc_space, loc_schedule, solar_thermal_system)
    ...
    model.getPlantLoops.each do |plant_loop|
      ...
      next unless (sys_id.is_initialized && sys_id.get == solar_thermal_system.water_heating_system.id)
    ...
    pump.additionalProperties.setFeature('HPXML_ID', solar_thermal_system.water_heating_system.id)
  ...
  def self.get_combi_boiler_and_plant_loop(model, heating_source_id)
    ...
    model.getPlantLoops.each do |pl|
      sys_id = pl.additionalProperties.getFeatureAsString('HPXML_ID')
  ...
  def self.add_ec_adj(model, heater, ec_adj, loc_space, water_heating_system, combi_boiler = nil, combi_hx = nil)
    ...
    ec_adj_output_var.additionalProperties.setFeature('FuelType', EPlus.fuel_type(fuel_type))
