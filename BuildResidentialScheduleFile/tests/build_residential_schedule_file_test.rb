...
class BuildResidentialScheduleFileTest < Minitest::Test
  ...
  def test_smooth
    ...
    sf = SchedulesFile.new(model: model, schedules_paths: hpxml.header.schedules_filepaths, col_names: ScheduleColumns.ColNames.keys)
  ...
  def test_water_heater_tank_scheduled
    ...
    sf = SchedulesFile.new(model: model, schedules_paths: hpxml.header.schedules_filepaths, col_names: ScheduleColumns.ColNames.keys, schedule_max_val: 150)
    sf.validate_schedules(year: 2007)

    assert(sf.schedules.keys.include?(ScheduleColumns.WaterHeaterSetpoint))
    assert(sf.schedules[ScheduleColumns.WaterHeaterSetpoint].include?(120))
    assert(sf.schedules[ScheduleColumns.WaterHeaterSetpoint].include?(125))
    assert(sf.schedules[ScheduleColumns.WaterHeaterSetpoint].include?(130))
    assert(!sf.schedules[ScheduleColumns.WaterHeaterSetpoint].include?(135))
    assert(sf.schedules[ScheduleColumns.WaterHeaterSetpoint].include?(140))
  end
  ...
  def test_water_heater_heat_pump_scheduled
    ...
    assert(!sf.schedules[ScheduleColumns.WaterHeaterSetpoint].include?(135))
    assert(sf.schedules[ScheduleColumns.WaterHeaterSetpoint].include?(140))
    assert(sf.schedules.keys.include?(ScheduleColumns.WaterHeaterOperatingMode))
    assert(sf.schedules[ScheduleColumns.WaterHeaterOperatingMode].include?(0))
    assert(sf.schedules[ScheduleColumns.WaterHeaterOperatingMode].include?(1))
  end
