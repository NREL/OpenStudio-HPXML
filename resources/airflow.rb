...
class Airflow
  ...
  def self.create_infil_mech_vent_objects(model, building, infil, mech_vent, wind_speed, tin_sensor, tout_sensor, vwind_sensor, duct_lks, wout_sensor, pbar_sensor)
    ...
    if mech_vent.type == 'central fan integrated supply'
      ...
      infil_program.addLine("    Set cfis_fan_runtime1 = @Max (@ABS(Minute - CFIS_t_fan_on)) (fan_rtf_hvac * ZoneTimeStep * 60)")
      # If fan_rtf_hvac, make sure it's not exceeding ventilation requirements
      infil_program.addLine("    Set cfis_fan_runtime = @Min cfis_fan_runtime1 (CFIS_t_min_hr_open - #{mech_vent.cfis_t_sum_open_var.name})")
      infil_program.addLine("    Set #{mech_vent.cfis_f_damper_open_var.name} = cfis_fan_runtime/(60.0*ZoneTimeStep)") # calculates the portion of the current timestep the CFIS damper needs to be open
      ...
      infil_program.addLine("    Set #{mech_vent.cfis_f_damper_extra_open_var.name} = @Max (#{mech_vent.cfis_f_damper_open_var.name}-fan_rtf_hvac) 0.0") # additional runtime fraction to meet min/hr
      ...
      infil_program.addLine("    Set cfis_fan_runtime2 = fan_rtf_hvac*ZoneTimeStep*60")
      infil_program.addLine("    If (#{mech_vent.cfis_t_sum_open_var.name}+cfis_fan_runtime2) > CFIS_t_min_hr_open")
      ...
      infil_program.addLine("      Set #{mech_vent.cfis_f_damper_open_var.name} = cfis_fan_runtime/(ZoneTimeStep*60)")
      infil_program.addLine("      Set QWHV = #{mech_vent.cfis_f_damper_open_var.name}*CFIS_Q_duct")
      ...
      infil_program.addLine("      Set #{mech_vent.cfis_f_damper_open_var.name} = fan_rtf_hvac")
      infil_program.addLine("      Set QWHV = #{mech_vent.cfis_f_damper_open_var.name}*CFIS_Q_duct")
      infil_program.addLine("      Set #{mech_vent.cfis_t_sum_open_var.name} = #{mech_vent.cfis_t_sum_open_var.name}+cfis_fan_runtime")
      infil_program.addLine("    EndIf")
...
class MechanicalVentilation
  ...
  attr_accessor(:type, :total_efficiency, :total_efficiency_adjusted, :whole_house_cfm, :fan_power_w, :sensible_efficiency, :sensible_efficiency_adjusted,
                ...
                :cfis_f_damper_open_var, :cfis_f_damper_extra_open_var, :cfis_fan_mfr_max_var, :cfis_fan_rtf_sensor, :cfis_fan_pressure_rise, :cfis_fan_efficiency,
