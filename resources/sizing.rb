...
class HVACSizing
  def self.apply(model:, 
    ...
    unless process_site_calcs_and_design_temps(runner: runner, building: building, weather: weather)
      return false
    end
    ...
    init_loads = aggregate_zone_loads(zones_loads: zones_loads)
  ...
  def self.process_site_calcs_and_design_temps(runner:,
                                               building:,
                                               weather:)
    '''
    ...
    # Initialize Manual J buffer zone temperatures using current design temperatures
    Geometry.get_thermal_zones(building: building).each do |thermal_zone|
      @cool_design_temps[thermal_zone] = process_design_temp_cooling(runner: runner,
                                                                     building: building,
                                                                     weather: weather,
                                                                     thermal_zone: thermal_zone)
      return false if @cool_design_temps[thermal_zone].nil?

      @heat_design_temps[thermal_zone] = process_design_temp_heating(runner: runner,
                                                                     building: building,
                                                                     weather: weather,
                                                                     thermal_zone: thermal_zone,
                                                                     design_db: weather.design.HeatingDrybulb)
      return false if @heat_design_temps[thermal_zone].nil?
    end

    return true
  end

  def self.process_design_temp_heating(runner:,
                                       building:,
                                       weather:,
                                       thermal_zone:,
                                       design_db:)
    if Geometry.thermal_zone_is_conditioned(thermal_zone: thermal_zone)
    ...
    elsif thermal_zone.include? "garage" # TODO
      # Garage
      heat_temp = design_db + 13

    elsif thermal_zone.include? "attic" # TODO

      is_vented = false
      if thermal_zone == "attic - vented"
        is_vented = true
      end

      attic_floor_r = self.get_thermal_zone_r_value(runner: runner, building: building, thermal_zone: thermal_zone, surface_type: "floor")
      return nil if attic_floor_r.nil?

      attic_roof_r = self.get_thermal_zone_r_value(runner: runner, building: building, thermal_zone: thermal_zone, surface_type: "roofceiling")
      ...
      if attic_floor_r < attic_roof_r
        ...
        if is_vented
          ...
          heat_temp = calculate_space_design_temps(runner: runner,
                                                   space: space,
                                                   weather: weather,
                                                   conditioned_design_temp: @conditioned_heat_design_temp,
                                                   design_db: design_db,
                                                   ground_db: weather.data.GroundMonthlyTemps.min)
        end
    ...
    if Geometry.thermal_zone_is_conditioned(thermal_zone: thermal_zone)
      ...
      if attic_floor_r < attic_roof_r
        ...
        if is_vented
          ...
          cool_temp = calculate_space_design_temps(runner: runner,
                                                   space: space,
                                                   weather: weather,
                                                   conditioned_design_temp: @conditioned_cool_design_temp,
                                                   design_db: weather.design.CoolingDrybulb,
                                                   ground_db: weather.data.GroundMonthlyTemps.max,
                                                   is_cooling_for_unvented_attic_roof_insulation: true)
        end
        ...
        building.elements.each("BuildingDetails/Enclosure/Roofs/Roof") do |roof|
          ...
          tot_roof_area += roof_values[:area]
          roof_color = "medium" # TODO
          roof_material = "asphalt shingles" # TODO
          has_radiant_barrier = roof_values[:radiant_barrier]

          if not is_vented
            if not has_radiant_barrier
              cool_temp += (150 + (weather.design.CoolingDrybulb - 95) + @daily_range_temp_adjust[@daily_range_num]) * roof_values[:area]
            else
              cool_temp += (130 + (weather.design.CoolingDrybulb - 95) + @daily_range_temp_adjust[@daily_range_num]) * roof_values[:area]
            end
  ...
  def self.process_zone_loads(runner:,
                              model:,
                              building:,
                              weather:)
    ...
    # TODO: Currently handles internal gains but not window loads.
    zones_sens = {}
    zones_lat = {}
    thermal_zones.each do |thermal_zone|
  ...
  def self.process_load_windows_skylights(runner:,
    ...
    Geometry.get_thermal_zone_above_grade_exterior_roofs(building: building, thermal_zone: thermal_zone).each do |roof_values|
      ...
      building.elements.each("BuildingDetails/Enclosure/Skylights/Skylight") do |skylight|
        ...
        u_skylight = skylight_values[:ufactor]
        return nil if u_skylight.nil?

        zone_loads.Heat_Skylights += u_skylight * skylight_values[:area] * @htd

        # SHGC & Internal Shading
        interior_shading_factor_summer = 1.0
        interior_shading_factor_winter = 1.0
        return nil if skylight_values[:shgc].nil?
        shgc_with_interior_shade_cool = skylight_values[:shgc] * interior_shading_factor_summer
        shgc_with_interior_shade_heat = skylight_values[:shgc] * interior_shading_factor_winter  

        skylightHasIntShading = true # TODO
  ...
  def self.process_load_doors(runner:,
                              building:,
                              thermal_zone:,
                              zone_loads:,
                              weather:)
    ...
    Geometry.get_thermal_zone_above_grade_exterior_walls(building: building, thermal_zone: thermal_zone).each do |wall_values|
      building.elements.each("BuildingDetails/Enclosure/Doors/Door") do |door|
        ...
        door_rvalue = door_values[:r_value]
        return nil if door_rvalue.nil?
        door_ufactor = 1.0 / door_rvalue # TODO
  ...
  def self.process_load_walls(runner:,
                              building:,
                              thermal_zone:,
                              zone_loads:,
                              weather:)
    ...
    Geometry.get_thermal_zone_below_grade_exterior_walls(building: building, thermal_zone: thermal_zone).each do |foundation_wall_values|
      ...
      u_value_mj8 = 0.0
      wall_height_ft = foundation_wall_values[:height].round
      ...
      zone_loads.Heat_Walls += u_value_mj8 * foundation_wall_values[:area] * @htd
    end
  ...
  def self.aggregate_zone_loads(zones_loads:)
    '''
    Intermediate Loads
  ...
  def self.calculate_hvac_temperatures(init_loads:,
                                       hvac:)
    '''
    HVAC Temperatures
    '''
  ...
  def self.apply_hvac_load_fractions(init_loads:,
                                     hvac:)
    '''
  ...
  def self.get_duct_regain_factor(runner:,
    ...
    if duct.LocationSpace.nil? # Outside
      ...
      infiltration_cfm = get_feature(runner: runner,
                                     obj: duct.LocationSpace.thermalZone.get,
                                     feature: Constants.SizingInfoZoneInfiltrationCFM,
                                     datatype: 'double',
  ...
  def self.process_duct_loads_heating(runner:,
    ...
    if init_heat_load == 0 or hvac.Ducts.nil? or hvac.Ducts.size == 0
      ...
      dse_As, dse_Ar = calc_ducts_areas(ducts: hvac.Ducts)
      supply_r, return_r = calc_ducts_rvalues(ducts: hvac.Ducts)

      design_temp_values = { Constants.DuctSideSupply => @heat_design_temps, Constants.DuctSideReturn => @heat_design_temps }
      dse_Tamb_heating_s, dse_Tamb_heating_r = calc_ducts_area_weighted_average(ducts: hvac.Ducts,
                                                                                values: design_temp_values)
      ...
      hvac.Ducts.each do |duct|
        dse_Fregains[duct.LocationSpace] = get_duct_regain_factor(runner: runner,
                                                                  duct: duct)
      ...
      dse_Fregain_s, dse_Fregain_r = calc_ducts_area_weighted_average(ducts: hvac.Ducts,
                                                                      values: fregain_values)

      # Initialize for the iteration
      delta = 1
      heatingLoad_Prev = init_heat_load
      heat_cfm = calc_airflow_rate(load_or_capacity: init_heat_load,
                                   deltaT: (hvac.SupplyAirTemp - @heat_setpoint))

      for _iter in 0..19
        break if delta.abs <= 0.001

        dse_Qs, dse_Qr = calc_ducts_leakages(ducts: hvac.Ducts,
                                             system_cfm: heat_cfm)

        dse_DE = calc_delivery_effectiveness_heating(dse_Qs: dse_Qs,
                                                     dse_Qr: dse_Qr,
                                                     system_cfm: heat_cfm,
                                                     load_sens: heatingLoad_Prev,
                                                     dse_Tamb_s: dse_Tamb_heating_s,
                                                     dse_Tamb_r: dse_Tamb_heating_r,
                                                     dse_As: dse_As,
                                                     dse_Ar: dse_Ar,
                                                     t_setpoint: @heat_setpoint,
                                                     dse_Fregain_s: dse_Fregain_s,
                                                     dse_Fregain_r: dse_Fregain_r,
                                                     supply_r: supply_r,
                                                     return_r: return_r)
        ...
        heat_cfm = calc_airflow_rate(load_or_capacity: heatingLoad_Next,
                                     deltaT: (hvac.SupplyAirTemp - @heat_setpoint))
  ...
  def self.process_duct_loads_cooling(runner:,
    ...
    if init_cool_load_sens == 0 or hvac.Ducts.nil? or hvac.Ducts.size == 0
      ...
      dse_Tamb_cooling_s, dse_Tamb_cooling_r = calc_ducts_area_weighted_average(ducts: hvac.Ducts,
                                                                                values: design_temp_values)
      ...
      initial_Cool_Airflow = calc_airflow_rate(load_or_capacity: init_cool_load_sens,
                                               deltaT: (@cool_setpoint - hvac.LeavingAirTemp))

      supply_leakage_cfm, return_leakage_cfm = calc_ducts_leakages(ducts: hvac.Ducts,
                                                                   system_cfm: initial_Cool_Airflow)

      hvac_final_values.Cool_Load_Lat, hvac_final_values.Cool_Load_Sens = calculate_sensible_latent_split(return_leakage_cfm: return_leakage_cfm,
                                                                                                          cool_load_tot: coolingLoad_Tot_Next,
                                                                                                          coolingLoadLat: init_cool_load_lat)
      ...
      for _iter in 1..50
        ...
        cool_Airflow = calc_airflow_rate(load_or_capacity: hvac_final_values.Cool_Load_Sens,
                                         deltaT: (@cool_setpoint - hvac.LeavingAirTemp))
        ...
        dse_Qs, dse_Qr = calc_ducts_leakages(ducts: hvac.Ducts,
                                             system_cfm: cool_Airflow)

        dse_DE, dse_dTe_cooling, hvac_final_values.Cool_Load_Ducts_Sens = calc_delivery_effectiveness_cooling(dse_Qs: dse_Qs,
                                                                                                              dse_Qr: dse_Qr,
                                                                                                              leavingAirTemp: hvac.LeavingAirTemp,
                                                                                                              system_cfm: cool_Airflow,
                                                                                                              load_sens: hvac_final_values.Cool_Load_Sens,
                                                                                                              dse_Tamb_s: dse_Tamb_cooling_s,
                                                                                                              dse_Tamb_r: dse_Tamb_cooling_r,
                                                                                                              dse_As: dse_As,
                                                                                                              dse_Ar: dse_Ar,
                                                                                                              t_setpoint: @cool_setpoint,
                                                                                                              dse_Fregain_s: dse_Fregain_s,
                                                                                                              dse_Fregain_r: dse_Fregain_r,
                                                                                                              load_total: hvac_final_values.Cool_Load_Tot,
                                                                                                              dse_h_r: dse_h_r,
                                                                                                              supply_r: supply_r,
                                                                                                              return_r: return_r)
    ...
    hvac_final_values.Cool_Airflow = calc_airflow_rate(load_or_capacity: hvac_final_values.Cool_Load_Sens,
                                                       deltaT: (@cool_setpoint - hvac.LeavingAirTemp))
  ...
  def self.process_cooling_equipment_adjustments(runner:,
                                                 hvac_final_values:,
                                                 weather:,
                                                 hvac:)
    '''
    ...
    if hvac.has_type([Constants.ObjectNameCentralAirConditioner,
      ...
      if hvac.has_type([Constants.ObjectNameCentralAirConditioner,
        ...
        hvac_final_values.SensibleCap_CurveValue = process_curve_fit(airFlowRate: hvac_final_values.Cool_Airflow,
                                                                     capacity: hvac_final_values.Cool_Load_Tot,
                                                                     temp: enteringTemp)
        ...
        if latCap_Design < hvac_final_values.Cool_Load_Lat
          ...
          hvac_final_values.Cool_Airflow = calc_airflow_rate(load_or_capacity: cool_Load_SensCap_Design,
                                                             deltaT: (@cool_setpoint - hvac.LeavingAirTemp))
  ...
  def self.process_fixed_equipment(runner:,
                                   hvac_final_values:,
                                   hvac:)
    '''
  ...
  def self.process_finalize(runner:,
                            hvac_final_values:,
                            zones_loads:,
                            weather:,
                            hvac:)
    ...
    if hvac.has_type(Constants.ObjectNameFurnace)
      ...
      hvac_final_values.Heat_Airflow = calc_airflow_rate(load_or_capacity: hvac_final_values.Heat_Capacity,
                                                         deltaT: (hvac.SupplyAirTemp - @heat_setpoint))
      ...
      if hvac_final_values.Cool_Capacity > @min_cooling_capacity
        ...
        hvac_final_values.Heat_Airflow = calc_airflow_rate(load_or_capacity: hvac_final_values.Heat_Capacity_Supp,
                                                           deltaT: (hvac.SupplyAirTemp - @heat_setpoint))
      end
  ...
  def self.process_heat_pump_adjustment(runner:,
                                        hvac_final_values:,
                                        weather:,
                                        hvac:)
    '''
  ...
  def self.process_slave_zone_flow_ratios(runner:,
                                          zones_loads:)
    '''
  ...
  def self.get_ventilation_rates(runner:,
    ...
    if mechVentType == Constants.VentTypeExhaust
      ...
      totalEfficiency = get_feature(runner: runner,
                                    obj: model.getBuilding,
                                    feature: Constants.SizingInfoMechVentTotalEfficiency,
                                    datatype: 'double')
      apparentSensibleEffectiveness = get_feature(runner: runner,
                                                  obj: model.getBuilding,
                                                  feature: Constants.SizingInfoMechVentApparentSensibleEffectiveness,
                                                  datatype: 'double')
      latentEffectiveness = get_feature(runner: runner,
                                        obj: model.getBuilding,
                                        feature: Constants.SizingInfoMechVentLatentEffectiveness,
                                        datatype: 'double')
  ...
  def self.get_fenestration_shgc(runner:,
                                 surface:)
    simple_glazing = self.get_window_simple_glazing(runner: runner,
                                                    surface: surface,
                                                    register_error: true)
  ...
  def self.calc_airflow_rate(load_or_capacity:,
                             deltaT:)
    return load_or_capacity / (1.1 * @acf * deltaT)
  end

  def self.calc_delivery_effectiveness_heating(dse_Qs:,
                                               dse_Qr:,
                                               system_cfm:,
                                               load_sens:,
                                               dse_Tamb_s:,
    ...
    '''
    dse_Bs, dse_Br, dse_a_s, dse_a_r, dse_dTe, dse_dT_s, dse_dT_r = _calc_dse_init(system_cfm: system_cfm,
                                                                                   load_sens: load_sens,
                                                                                   dse_Tamb_s: dse_Tamb_s,
                                                                                   dse_Tamb_r: dse_Tamb_r,
                                                                                   dse_As: dse_As,
                                                                                   dse_Ar: dse_Ar,
                                                                                   ...
                                                                                   supply_r: supply_r,
                                                                                   return_r: return_r)
    dse_DE = _calc_dse_DE_heating(dse_a_s: dse_a_s,
                                  dse_Bs: dse_Bs,
                                  dse_a_r: dse_a_r,
                                  dse_Br: dse_Br,
                                  dse_dT_s: dse_dT_s,
                                  dse_dT_r: dse_dT_r,
                                  dse_dTe: dse_dTe)
    dse_DEcorr = _calc_dse_DEcorr(dse_DE: dse_DE,
                                  dse_Fregain_s: dse_Fregain_s,
                                  dse_Fregain_r: dse_Fregain_r,
                                  dse_Br: dse_Br,
                                  dse_a_r: dse_a_r,
  ...
  def self.calc_delivery_effectiveness_cooling(dse_Qs:,
                                               dse_Qr:,
                                               leavingAirTemp:,
                                               system_cfm:,
                                               load_sens:,
    ...
    dse_Bs, dse_Br, dse_a_s, dse_a_r, dse_dTe, dse_dT_s, dse_dT_r = _calc_dse_init(system_cfm: system_cfm,
                                                                                   ...
                                                                                   return_r: return_r)
    dse_dTe *= -1
    dse_DE, coolingLoad_Ducts_Sens = _calc_dse_DE_cooling(dse_a_s: dse_a_s,
                                                          system_cfm: system_cfm,
  ...
  def self._calc_dse_init(system_cfm:,
                          load_sens:,
                          dse_Tamb_s:,
                          dse_Tamb_r:,
                          dse_As:,
  ...
  def self._calc_dse_DE_cooling(dse_a_s:,
                                system_cfm:,
                                load_total:,
                                dse_a_r:,
                                dse_h_r:,
  ...
  def self._calc_dse_DE_heating(dse_a_s:,
                                dse_Bs:,
                                dse_a_r:,
                                dse_Br:,
                                dse_dT_s:,
  ...
  def self._calc_dse_DEcorr(dse_DE:,
                            dse_Fregain_s:,
                            dse_Fregain_r:,
                            dse_Br:,
                            dse_a_r:,
  ...
  def self.calculate_sensible_latent_split(return_leakage_cfm:,
                                           cool_load_tot:,
                                           coolingLoadLat:)
  ...
  def self.get_ducts_for_air_loop(runner:,
                                  air_loop:)
    ducts = []

    # Has ducts?
    has_ducts = get_feature(runner: runner,
                            obj: air_loop,
                            feature: Constants.SizingInfoDuctExist,
                            datatype: 'boolean')
    return ducts unless has_ducts

    # Leakage values
    leakage_fracs = get_feature(runner: runner,
                                obj: air_loop,
                                feature: Constants.SizingInfoDuctLeakageFracs,
                                datatype: 'string')
    leakage_cfm25s = get_feature(runner: runner,
                                 obj: air_loop,
                                 feature: Constants.SizingInfoDuctLeakageCFM25s,
                                 datatype: 'string')
    ...
    # Areas
    areas = get_feature(runner: runner,
                        obj: air_loop,
                        feature: Constants.SizingInfoDuctAreas,
                        datatype: 'string')
    ...
    # R-values
    rvalues = get_feature(runner: runner,
                          obj: air_loop,
                          feature: Constants.SizingInfoDuctRvalues,
                          datatype: 'string')
    ...
    # Locations
    locations = get_feature(runner: runner,
                            obj: air_loop,
                            feature: Constants.SizingInfoDuctLocationZones,
                            datatype: 'string')
    ...
    # Sides
    sides = get_feature(runner: runner,
                        obj: air_loop,
                        feature: Constants.SizingInfoDuctSides,
                        datatype: 'string')
  ...
  def self.calc_ducts_leakages(ducts:,
                               system_cfm:)
    '''
  ...
  def self.calc_ducts_rvalues(ducts:)
    ...
    supply_u, return_u = calc_ducts_area_weighted_average(ducts: ducts,
                                                          values: u_factors)
  ...
  def self.get_hvacs(runner:,
    ...
    equips.each do |equip, control_zone|
      ...
      if not clg_coil.nil?
        ratedCFMperTonCooling = get_feature(runner: runner,
                                            obj: equip,
                                            feature: Constants.SizingInfoHVACRatedCFMperTonCooling,
                                            datatype: 'string',
                                            register_error: false)
        ...
        hvac.CoolingLoadFraction = get_feature(runner: runner,
                                               obj: equip,
                                               feature: Constants.SizingInfoHVACFracCoolLoadServed,
                                               datatype: 'double')
      ...
      if clg_coil.is_a? OpenStudio::Model::CoilCoolingDXSingleSpeed
        ...
        if hvac.has_type(Constants.ObjectNameRoomAirConditioner)
          coolingCFMs = get_feature(runner: runner,
                                    obj: equip,
                                    feature: Constants.SizingInfoHVACCoolingCFMs,
                                    datatype: 'string')
        ...
        hvac.COOL_CAP_FT_SPEC = get_2d_vector_from_CAP_FT_SPEC_curves(curves: curves,
                                                                      num_speeds: hvac.NumSpeedsCooling)
        ...
        capacityRatioCooling = get_feature(runner: runner,
                                           obj: equip,
                                           feature: Constants.SizingInfoHVACCapacityRatioCooling,
                                           datatype: 'string')
        ...
        cOIL_BF_FT_SPEC = get_feature(runner: runner,
                                      obj: equip,
                                      feature: Constants.SizingInfoGSHPCoil_BF_FT_SPEC,
                                      datatype: 'string')
        return nil if cOIL_BF_FT_SPEC.nil?

        hvac.COIL_BF_FT_SPEC = [cOIL_BF_FT_SPEC.split(",").map(&:to_f)]

        shr_rated = get_feature(runner: runner,
                                obj: equip,
                                feature: Constants.SizingInfoHVACSHR,
                                datatype: 'string')
        return nil if shr_rated.nil?

        hvac.SHRRated = shr_rated.split(",").map(&:to_f)

        hvac.CoilBF = get_feature(runner: runner,
                                  obj: equip,
                                  feature: Constants.SizingInfoGSHPCoilBF,
                                  datatype: 'double')
        ...
        hvac.GSHP_BoreSpacing = get_feature(runner: runner, obj: equip, feature: Constants.SizingInfoGSHPBoreSpacing, datatype: 'double')
        hvac.GSHP_BoreHoles = get_feature(runner: runner, obj: equip, feature: Constants.SizingInfoGSHPBoreHoles, datatype: 'string')
        hvac.GSHP_BoreDepth = get_feature(runner: runner, obj: equip, feature: Constants.SizingInfoGSHPBoreDepth, datatype: 'string')
        hvac.GSHP_BoreConfig = get_feature(runner: runner, obj: equip, feature: Constants.SizingInfoGSHPBoreConfig, datatype: 'string')
        hvac.GSHP_SpacingType = get_feature(runner: runner, obj: equip, feature: Constants.SizingInfoGSHPUTubeSpacingType, datatype: 'string')
      ...
      if not htg_coil.nil?
        if hvac.has_type(Constants.ObjectNameAirSourceHeatPump)
          hvac.HPSizedForMaxLoad = get_feature(runner: runner, obj: equip, feature: Constants.SizingInfoHPSizedForMaxLoad, datatype: 'boolean')
          return nil if hvac.HPSizedForMaxLoad.nil?
        end

        ratedCFMperTonHeating = get_feature(runner: runner, obj: equip, feature: Constants.SizingInfoHVACRatedCFMperTonHeating, datatype: 'string', register_error: false)
        if not ratedCFMperTonHeating.nil?
          hvac.RatedCFMperTonHeating = ratedCFMperTonHeating.split(",").map(&:to_f)
        end

        hvac.HeatingLoadFraction = get_feature(runner: runner, obj: equip, feature: Constants.SizingInfoHVACFracHeatLoadServed, datatype: 'double')
      ...
      if equip.is_a? OpenStudio::Model::ZoneHVACBaseboardConvectiveElectric
        ...
        hvac.HEAT_CAP_FT_SPEC = get_2d_vector_from_CAP_FT_SPEC_curves(curves: curves,
                                                                      num_speeds: hvac.NumSpeedsHeating)
        ...
        capacityRatioHeating = get_feature(runner: runner, obj: equip, feature: Constants.SizingInfoHVACCapacityRatioHeating, datatype: 'string')
        ...
        if hvac.HeatType == Constants.ObjectNameMiniSplitHeatPump
          heatingCFMs = get_feature(runner: runner, obj: equip, feature: Constants.SizingInfoHVACHeatingCFMs, datatype: 'string')
          return nil if heatingCFMs.nil?

          hvac.HeatingCFMs = heatingCFMs.split(",").map(&:to_f)

          hvac.HeatingCapacityOffset = get_feature(runner: runner, obj: equip, feature: Constants.SizingInfoHVACHeatingCapacityOffset, datatype: 'double')
    ...
    hvacs.each do |hvac|
      hvac.Objects.each do |object|
        other_zone_object = get_feature(runner: runner, obj: object, feature: Constants.SizingInfoHVACOtherZoneObject, datatype: 'string', register_error: false)
  ...
  def self.get_2d_vector_from_CAP_FT_SPEC_curves(curves:,
                                                 num_speeds:)
    vector = []
  ...
  def self.process_curve_fit(airFlowRate:,
                             capacity:,
                             temp:)
  ...
  def self.space_is_vented(space:,
                           min_sla_for_venting:)
    ela = 0.0
  ...
  def self.calculate_t_attic_iter(runner:,
                                  attic_UAs:,
                                  t_solair:,
                                  cool_setpoint:,
                                  coolingLoad_Ducts_Sens:)
  ...
  def self.calculate_t_solair(weather:,
                              roofAbsorptance:,
                              roofPitch:)
  ...
  def self.get_space_ua_values(runner:,
    ...
    space.surfaces.each do |surface|
      ...
      if obc == "foundation"
        ...
        ufactor = self.get_surface_ufactor(runner: runner,
                                           surface: surface,
    ...
    # Infiltration UA
    infiltration_cfm = get_feature(runner: runner, obj: space.thermalZone.get, feature: Constants.SizingInfoZoneInfiltrationCFM, datatype: 'double', register_error: false)
    infiltration_cfm = 0 if infiltration_cfm.nil?
  ...
  def self.calculate_space_design_temps(runner:,
    ...
    space_UAs = get_space_ua_values(runner: runner,
                                    space: space,
                                    weather: weather)
  ...
  def self.gshp_hx_pipe_rvalue(pipe_od:,
                               pipe_id:,
                               pipe_cond:)
  ...
  def self.gshp_gfnc_coeff(bore_config:,
                           num_bore_holes:,
  ...
  def self.get_foundation_walls_ceilings_insulated(runner:,
    ...
    space.surfaces.each do |surface|
      ...
      ceiling_ufactor = self.get_surface_ufactor(runner: runner,
                                                 surface: surface,
                                                 surface_type: surface.surfaceType,
                                                 register_error: true)
    end
  ...
  def self.set_object_values(runner:,
    ...
    hvac.Objects.each do |object|
      ...
      if object.is_a? OpenStudio::Model::AirLoopHVACUnitarySystem and object.airLoopHVAC.is_initialized
        ...
        # Coils
        setCoilsObjectValues(runner: runner,
                             model: model,
                             hvac: hvac,
                             equip: object,
        ...
        setCoilsObjectValues(runner: runner,
                             ...
                             equip: object,
                             hvac_final_values: hvac_final_values,
                             zone_ratio: zone_ratios[thermal_zone])
  ...
  def self.setCoilsObjectValues(runner:,
                                model:,
                                hvac:,
                                equip:,
                                hvac_final_values:,
    ...
    # Cooling coil
    if clg_coil.is_a? OpenStudio::Model::CoilCoolingDXSingleSpeed
      clg_coil.setRatedTotalCoolingCapacity(zone_ratio * UnitConversions.convert(hvac_final_values.Cool_Capacity, "Btu/hr", "W"))
      clg_coil.setRatedAirFlowRate(zone_ratio * UnitConversions.convert(hvac_final_values.Cool_Capacity, "Btu/hr", "ton") * UnitConversions.convert(hvac.RatedCFMperTonCooling[0], "cfm", "m^3/s"))

    elsif clg_coil.is_a? OpenStudio::Model::CoilCoolingDXMultiSpeed
      clg_coil.stages.each_with_index do |stage, speed|
        stage.setGrossRatedTotalCoolingCapacity(zone_ratio * UnitConversions.convert(hvac_final_values.Cool_Capacity, "Btu/hr", "W") * hvac.CapacityRatioCooling[speed])
        if clg_coil.name.to_s.start_with? Constants.ObjectNameAirSourceHeatPump or clg_coil.name.to_s.start_with? Constants.ObjectNameCentralAirConditioner
          stage.setRatedAirFlowRate(zone_ratio * UnitConversions.convert(hvac_final_values.Cool_Capacity, "Btu/hr", "ton") * UnitConversions.convert(hvac.RatedCFMperTonCooling[speed], "cfm", "m^3/s") * hvac.CapacityRatioCooling[speed])
        elsif clg_coil.name.to_s.start_with? Constants.ObjectNameMiniSplitHeatPump
          stage.setRatedAirFlowRate(zone_ratio * UnitConversions.convert(hvac_final_values.Cool_Capacity, "Btu/hr", "ton") * UnitConversions.convert(hvac.CoolingCFMs[speed], "cfm", "m^3/s"))
        end
      end

    elsif clg_coil.is_a? OpenStudio::Model::CoilCoolingWaterToAirHeatPumpEquationFit
      clg_coil.setRatedAirFlowRate(zone_ratio * UnitConversions.convert(hvac_final_values.Cool_Airflow, "cfm", "m^3/s"))
      clg_coil.setRatedWaterFlowRate(zone_ratio * UnitConversions.convert(hvac_final_values.GSHP_Loop_flow, "gal/min", "m^3/s"))
      clg_coil.setRatedTotalCoolingCapacity(zone_ratio * UnitConversions.convert(hvac_final_values.Cool_Capacity, "Btu/hr", "W"))
      clg_coil.setRatedSensibleCoolingCapacity(zone_ratio * UnitConversions.convert(hvac_final_values.Cool_Capacity_Sens, "Btu/hr", "W"))
    ...
    if htg_coil.is_a? OpenStudio::Model::CoilHeatingElectric
      if not equip.is_a? OpenStudio::Model::ZoneHVACPackagedTerminalAirConditioner
        htg_coil.setNominalCapacity(zone_ratio * UnitConversions.convert(hvac_final_values.Heat_Capacity, "Btu/hr", "W"))
      end
      ...
      htg_coil.setRatedTotalHeatingCapacity(zone_ratio * UnitConversions.convert(hvac_final_values.Heat_Capacity, "Btu/hr", "W"))
      htg_coil.setRatedAirFlowRate(zone_ratio * UnitConversions.convert(hvac_final_values.Heat_Capacity, "Btu/hr", "ton") * UnitConversions.convert(hvac.RatedCFMperTonHeating[0], "cfm", "m^3/s"))

    elsif htg_coil.is_a? OpenStudio::Model::CoilHeatingDXMultiSpeed
      htg_coil.stages.each_with_index do |stage, speed|
        stage.setGrossRatedHeatingCapacity(zone_ratio * UnitConversions.convert(hvac_final_values.Heat_Capacity, "Btu/hr", "W") * hvac.CapacityRatioHeating[speed])
        if htg_coil.name.to_s.start_with? Constants.ObjectNameAirSourceHeatPump
          stage.setRatedAirFlowRate(zone_ratio * UnitConversions.convert(hvac_final_values.Heat_Capacity, "Btu/hr", "ton") * UnitConversions.convert(hvac.RatedCFMperTonHeating[speed], "cfm", "m^3/s") * hvac.CapacityRatioHeating[speed])
        elsif htg_coil.name.to_s.start_with? Constants.ObjectNameMiniSplitHeatPump
          stage.setRatedAirFlowRate(zone_ratio * UnitConversions.convert(hvac_final_values.Heat_Capacity, "Btu/hr", "ton") * UnitConversions.convert(hvac.HeatingCFMs[speed], "cfm", "m^3/s"))
        end
      end

    elsif htg_coil.is_a? OpenStudio::Model::CoilHeatingWaterToAirHeatPumpEquationFit
      htg_coil.setRatedAirFlowRate(zone_ratio * UnitConversions.convert(hvac_final_values.Heat_Airflow, "cfm", "m^3/s"))
      htg_coil.setRatedWaterFlowRate(zone_ratio * UnitConversions.convert(hvac_final_values.GSHP_Loop_flow, "gal/min", "m^3/s"))
      htg_coil.setRatedHeatingCapacity(zone_ratio * UnitConversions.convert(hvac_final_values.Heat_Capacity, "Btu/hr", "W"))
  ...
  def self.get_space_r_value(runner:,
                             space:,
                             surface_type:,
                             register_error: false)
  ...
  def self.get_thermal_zone_r_value(runner:,
                                    building:,
                                    thermal_zone:,
                                    surface_type:)
    # Get area-weighted space r-value
    ...
    if surface_type == "floor"
      building.elements.each("BuildingDetails/Enclosure/FrameFloors/FrameFloor") do |framefloor|
        ...
        ufactor = 1.0 / framefloor_values[:insulation_assembly_r_value] # TODO
        next if ufactor.nil?
      ...
      building.elements.each("BuildingDetails/Enclosure/Roofs/Roof") do |roof|
        ...
        ufactor = 1.0 / roof_values[:insulation_assembly_r_value] # TODO
        next if ufactor.nil?
  ...
  def self.get_surface_ufactor(runner:,
                               surface:,
                               surface_type:,
                               register_error: false)
  ...
  def self.get_window_simple_glazing(runner:,
                                     surface:,
                                     register_error: false)
    if not surface.construction.is_initialized
      if register_error
  ...
  def self.display_hvac_final_values_results(runner:,
                                             hvac_final_values:)
    s = "Final Results:"
    loads = [
