...
class Lighting
  def self.apply(model, runner, weather, interior_kwh, garage_kwh, exterior_kwh, cfa, gfa,
    ...
    # Create schedule
    sch = HourlyByMonthSchedule.new(model, runner, Constants.ObjectNameLighting + " schedule", lighting_sch, lighting_sch, normalize_values = true, create_sch_object = true, lower_limit_value = 0, upper_limit_value = 1, numeric_type = "Continuous")
