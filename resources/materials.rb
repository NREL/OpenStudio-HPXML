...
class Material
  ...
  def self.AirFilmSlopeEnhanced(roof_pitch)
    ...
    # emissivity = 0.90.
    roof_pitch_degrees = UnitConversions.convert(Math.atan(roof_pitch / 12.0), "rad", "deg")
    rvalue = 0.002 * Math::exp(0.0398 * roof_pitch_degrees) + 0.608 # hr-ft-F/Btu (evaluates to film_flat_enhanced at 0 degrees, 0.62 at 45 degrees, and film_vertical at 90 degrees)
    return self.new(name = Constants.AirFilm, thick_in = 1.0, mat_base = nil, k_in = 1.0 / rvalue)
  end
  ...
  def self.AirFilmSlopeReduced(roof_pitch)
    ...
    # emissivity = 0.90.
    roof_pitch_degrees = UnitConversions.convert(Math.atan(roof_pitch / 12.0), "rad", "deg")
    rvalue = 0.32 * Math::exp(-0.0154 * roof_pitch_degrees) + 0.6 # hr-ft-F/Btu (evaluates to film_flat_reduced at 0 degrees, 0.76 at 45 degrees, and film_vertical at 90 degrees)
    return self.new(name = Constants.AirFilm, thick_in = 1.0, mat_base = nil, k_in = 1.0 / rvalue)
  end
  ...
  def self.AirFilmSlopeEnhancedReflective(roof_pitch)
    ...
    # emissivity = 0.05.
    roof_pitch_degrees = UnitConversions.convert(Math.atan(roof_pitch / 12.0), "rad", "deg")
    rvalue = 0.00893 * Math::exp(0.0419 * roof_pitch_degrees) + 1.311 # hr-ft-F/Btu (evaluates to 1.32 at 0 degrees, 1.37 at 45 degrees, and 1.70 at 90 degrees)
    return self.new(name = Constants.AirFilm, thick_in = 1.0, mat_base = nil, k_in = 1.0 / rvalue)
  end
  ...
  def self.AirFilmSlopeReducedReflective(roof_pitch)
    ...
    # emissivity = 0.05.
    roof_pitch_degrees = UnitConversions.convert(Math.atan(roof_pitch / 12.0), "rad", "deg")
    rvalue = 2.999 * Math::exp(-0.0333 * roof_pitch_degrees) + 1.551 # hr-ft-F/Btu (evaluates to 4.55 at 0 degrees, 2.22 at 45 degrees, and 1.70 at 90 degrees)
    return self.new(name = Constants.AirFilm, thick_in = 1.0, mat_base = nil, k_in = 1.0 / rvalue)
  end
