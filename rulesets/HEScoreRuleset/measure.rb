...
class HEScoreMeasure < OpenStudio::Measure::ModelMeasure
  ...
  def arguments(model)
    ...
    arg = OpenStudio::Measure::OSArgument.makeStringArgument('input_file_path', true)
    arg.setDisplayName('Input File Path')
    arg.setDescription('Absolute (or relative) path of the HPXML/JSON file.')
    args << arg
  ...
  def run(model, runner, user_arguments)
    ...
    input_file_path = runner.getStringArgumentValue('input_file_path', user_arguments)
    hpxml_output_path = runner.getOptionalStringArgumentValue('hpxml_output_path', user_arguments)

    unless (Pathname.new input_file_path).absolute?
      input_file_path = File.expand_path(File.join(File.dirname(__FILE__), input_file_path))
    end
    unless File.exist?(input_file_path)
      if input_file_path.downcase.end_with?('.xml')
        runner.registerError("'#{input_file_path}' does not exist or is not an .xml file.")
        return false
      elsif input_file_path.downcase.end_with?('.json')
        runner.registerError("'#{input_file_path}' does not exist or is not a .json file.")
        return false
      end
    end

    if input_file_path.downcase.end_with?('.json')
      # TODO: run json_to_hpxml and return hpxml_path here
      input_file_path = 'C:\GitHub\OpenStudio-HEScore\workflow\sample_files\Base_hpxml.xml'  # TODO: replace it with the hpxml_path generated by json_to_hpxml
    end

    hpxml = HPXML.new(hpxml_path: input_file_path, collapse_enclosure: false)
