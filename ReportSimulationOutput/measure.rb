...
class ReportSimulationOutput < OpenStudio::Measure::ReportingMeasure
  ...
  def append_sizing_results(results_out, line_break)
    ...
    @hpxml_bldgs.each do |hpxml_bldg|
      hpxml_bldg.zones.map { |z| z.spaces }.flatten.each do |space|
        results_out << [line_break]
        # Note: Latent loads are not calculated for spaces
        results_out << ["HVAC Room Design Load: #{space.id}: Heating: Total (Btu/h)", space.hdl_total.round(1)]
        results_out << ["HVAC Room Design Load: #{space.id}: Heating: Ducts (Btu/h)", space.hdl_ducts.round(1)]
        results_out << ["HVAC Room Design Load: #{space.id}: Heating: Windows (Btu/h)", space.hdl_windows.round(1)]
        results_out << ["HVAC Room Design Load: #{space.id}: Heating: Skylights (Btu/h)", space.hdl_skylights.round(1)]
        results_out << ["HVAC Room Design Load: #{space.id}: Heating: Doors (Btu/h)", space.hdl_doors.round(1)]
        results_out << ["HVAC Room Design Load: #{space.id}: Heating: Walls (Btu/h)", space.hdl_walls.round(1)]
        results_out << ["HVAC Room Design Load: #{space.id}: Heating: Roofs (Btu/h)", space.hdl_roofs.round(1)]
        ...
        results_out << ["HVAC Room Design Load: #{space.id}: Cooling Sensible: Skylights (Btu/h)", space.cdl_sens_skylights.round(1)]
        results_out << ["HVAC Room Design Load: #{space.id}: Cooling Sensible: Doors (Btu/h)", space.cdl_sens_doors.round(1)]
        results_out << ["HVAC Room Design Load: #{space.id}: Cooling Sensible: Walls (Btu/h)", space.cdl_sens_walls.round(1)]
        results_out << ["HVAC Room Design Load: #{space.id}: Cooling Sensible: Roofs (Btu/h)", space.cdl_sens_roofs.round(1)]
